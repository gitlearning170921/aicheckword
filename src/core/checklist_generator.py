...
12. **医疗器械软件：器械法规与软件生命周期双覆盖（优先）**：须包含或强化审核点，使清单**同时**覆盖（1）**医疗器械监管侧**：与注册类别、适用范围、风险管理与残余风险、说明书/标签、临床评价路径（若适用）、质量管理体系对软件相关活动的受控要求等（以参考知识中的法规、指导原则、程序文件为准，**禁止编造条款号**）；（2）**软件工程与生命周期侧**：需求—设计—实现—验证—发布—维护的文档化与可追溯、配置管理与版本标识、缺陷与变更、网络安全与数据（若适用）、现成软件/SOUP（若适用）。`regulation_ref` 可并列引用参考知识中出现的 **YY/T 0664、IEC 62304、ISO 14971、ISO 13485、药监局技术审查指导原则** 等，须与参考知识实际内容对应。
13. **验证与确认层次与测试证据（医疗器械软件，优先）**：须包含或强化审核点：单元/集成/系统/确认（或用户验收）等测试层次在文档中是否**界定清楚**、与风险及需求级别是否相称；测试计划、用例、报告/记录是否形成**可支撑的验证证据链**（在文档类型范围内）。check_method 须指导审核员按需求与风险条目检查测试覆盖与记录完整性，识别「有需求无对应验证思路/记录」类缺口。
14. **欧盟/美国注册文档语言（优先）**：须包含或强化审核点：当注册国家为**欧盟或美国**时，核查待审文档是否为英文，或是否提供受控且可追溯的英文版本/翻译件；若仅中文且无英文受控版本，或中英文版本关键术语（产品名称、型号、适用范围、风险术语、接口术语等）不一致，须判定为合规性/一致性问题。check_method 须指导审核员核对文档语言属性、版本关系、关键术语对照表与受控记录。applicable_docs 可包含「软件需求规范」「软件描述文档」「风险管理报告」「软件可追溯性分析报告」「说明书」「标签」等。
15. **引用工具/组件版本与时间逻辑（优先，医疗器械软件）**：须包含或强化审核点：文档中出现的**工具、框架、操作系统、数据库、现成软件/SOUP、第三方库、测试/构建工具**等**具体版本号**，其**公开发布/GA 时间**是否**不晚于**文档声称使用该工具/环境的**时间点**（开发、测试、验证、编制、生效、修订履历等）；若版本在文档时间点尚未发布，须作为准确性/一致性问题。check_method 须指导审核员提取「工具名+版本号+文档日期」三元组逐项对照；发布日期须引用可核对来源（官方说明、知识库 SOUP 清单等），**禁止臆造**；无法核实时建议补充证据。applicable_docs 可包含「软件开发环境说明」「测试报告」「验证报告」「SOUP 清单」「配置管理记录」「软件描述文档」等。
...
12. **医疗器械软件：器械法规与软件生命周期双覆盖**：若现有清单缺少针对「同时从医疗器械监管要求与软件生命周期工程要求审视文档」的审核点，须补充；若已有则完善 check_method：指导审核员分别核对风险管理/说明书与标签/质量体系对软件的相关要求，以及需求、设计、验证确认、配置与变更、网络安全与 SOUP（若适用）等是否文档化且可追溯。`regulation_ref` 须与参考知识一致，禁止编造条款号。
13. **验证与确认层次与测试证据**：若现有清单缺少针对「测试层次、测试记录与需求/风险相称性」的审核点，须补充；若已有则完善 check_method：核对单元/集成/系统/确认测试等是否界定清楚、记录是否足以支撑结论。
14. **欧盟/美国注册文档语言**：若现有清单缺少针对「注册国家为欧盟/美国时文档英文要求」的审核点，须补充；若已有则完善 check_method：核对是否存在英文受控版本、翻译受控记录，以及中英文关键术语与核心声明的一致性。
15. **引用工具/组件版本与时间逻辑**：若现有清单缺少针对「文档引用工具/SOUP/运行环境具体版本号与其使用/测试/编制日期是否匹配公开发布时间」的审核点，须补充；若已有则完善 check_method：要求审核员提取工具名、版本号与文档日期对照，识别「版本未发布已引用」类矛盾；发布日期须可核对，禁止臆造。
