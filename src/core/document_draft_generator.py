...
def _filter_excerpt_by_intent(excerpt: str, *, target_name: str) -> str:
    ...
    # 负向关键词：安全/审计/权限等不应落入 compat/usability 等测试文档
    deny_security = [
        "网络安全",
        "安全",
        "security",
...
4. **重要——参考文件仅作技术内容来源**：INPUT_DOCS_EXCERPT 用于对齐**技术类**表述（定义与缩略语、法规/标准条目、风险与控制措施、权限/日志/流程等功能性描述等）。其中若出现其它案例项目的**公司名称、产品名称、型号规格、注册信息**等标识，**不得**当作本项目信息使用；项目标识一律以【项目约束】与 **NEW_BASIC_INFO** 为准。你必须仔细阅读参考内容，将其中与基底文档相关章节对应的技术要点逐项更新到对应位置，不能只更新部分要点而遗漏其余。
...
【任务目标】
- 你需要根据 INPUT_SYSTEM_FUNCTIONALITY 与 INPUT_DOCS_EXCERPT，检查 EXISTING_DRAFT_TEXT 中与"系统功能/软件功能/模块列表/界面说明/操作流程/功能说明/基本信息字段/权限/审计/日志"等相关的内容，判断哪些地方需要修改。**项目名称/产品名称/型号/注册单元**等基本信息字段的合法取值以 **NEW_BASIC_INFO** 与【项目约束】为准，不得因参考文件中的案例标识而改写。
...
- 若参考文件要求的是“新增信息”，而基础文档对应章节已存在标题/小节且需要在原章节中补充内容：
  ...
  - 若需要补充到原表格中（新增行）：可使用 `insert_table_row_after_contains`，以表头或某行关键单元格原文作为 anchor，在其后插入新行；**Excel（xlsx）** 下 `new_text` **必须用制表符 `\\t` 分列**，每列对应表格一列，列数与表头一致；**禁止**把整行多个字段不加 `\\t` 拼成一串（否则会全部写入首列）。程序在无 `\\t` 时会尝试启发式拆列，但不可靠，仍以模型正确分列为主。
...
- 每个 operation 必须包含：
  ...
  - new_text: 替换后的文本（可多行）。多条缩略语定义、多条 `[n]` 文献、多条法规/标准：用**换行**分隔；程序会展开为多段并继承锚点格式，在 References 类章节对 `[n]` 续号。
