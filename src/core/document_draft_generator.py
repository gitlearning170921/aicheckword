...
- 若参考文件要求的是“新增信息”，而基础文档对应章节已存在标题/小节且需要在原章节中补充内容：
  ...
  - 若需要补充到原表格中（新增行）：可使用 `insert_table_row_after_contains`，以表头或某行关键单元格原文作为 anchor，在其后插入新行。**禁止**把表格多行内容（含 GNxx-数字、SRS_xxx、制表符分列）用 `insert_paragraph_after_contains` 写入正文——否则 Word 会出现可见的 `\\t`/`\\n` 乱码且破坏版式。**多条用例可写在同一 operation**：`new_text` 内用**真实换行**分隔多行，每行用**真实制表符**分列（JSON 中写 `\\t`/`\\n` 即可，勿写成可见的反斜杠+t）；程序会按序逐行插入表格。 **推荐按表头对齐**：在同一条 operation 中提供 **row_by_header**（对象：键为与表头单元格相同或可部分匹配的列名，值为该列文本），或将 **new_text 设为 JSON 对象**（键值同上）；程序会在锚点行上方自动识别表头行并把各值写入对应列（**docx / xlsx 均支持**）。仍兼容用制表符 `\\t` 按**从左到右**分列：`new_text` 中 **Excel（xlsx）** 强烈建议用 `\\t` 且列数与表一致；**禁止**把多列不加 `\\t` 拼成一串（否则会挤进首列）。无 `\\t` 时程序会尽力启发式拆列，不可靠。
