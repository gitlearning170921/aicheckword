...
from .draft_export import normalize_patch_json_string
...
def _infer_intent_key(name: str) -> str:
    """从文件名/目录名推断意图：compat/usability/security/performance/fault/data/other。"""
    t = (name or "").lower()
    if any(k in t for k in ("兼容", "compatibility", "compat")):
        return "compat"
    if any(k in t for k in ("可用性", "usability", "ui", "ux")):
        return "usability"
    if any(k in t for k in ("网络安全", "安全", "security", "cyber", "vulnerability", "penetration", "threat")):
        return "security"
    if any(k in t for k in ("性能", "performance", "stress", "load")):
        return "performance"
    if any(k in t for k in ("容错", "tolerance", "fault", "robust", "robustness", "异常")):
        return "fault"
    if any(k in t for k in ("正确性", "correctness", "accuracy", "data")):
        return "data"
    return "other"


def _filter_excerpt_by_intent(excerpt: str, *, target_name: str) -> str:
    """
    对输入/参考摘要做“意图过滤”，避免在兼容性测试等文档中引入网络安全/审计类内容。
    规则以 target_name（含目录/文件名）推断 intent，并对明显不相关的段落块做剔除。
    """
    txt = (excerpt or "").strip()
    if not txt:
        return txt
    intent = _infer_intent_key(target_name or "")
    if intent == "other":
        return txt

    # 负向关键词：仅命中"明确属于网络安全/审计/访问控制"语义的窄词，
    # 避免"安全"这类宽词误删兼容性/可用性段落中正常出现的"参数安全/患者安全/设备安全/医疗安全"等内容。
    deny_security = [
        "网络安全",
        "信息安全",
        "安全漏洞",
    ...
    allow_compat = ["兼容", "compatibility", "compat", "browser", "windows", "chrome", "edge", "os", "操作系统"]
    allow_usability = ["可用性", "usability", "ui", "ux", "交互", "提示", "易用", "可读"]

    def _has_any(s: str, kws: List[str]) -> bool:
        sl = (s or "").lower()
        return any((k or "").lower() in sl for k in kws if (k or "").strip())

    parts = re.split(r"(?=【输入/参考文件：)", txt)
    kept: List[str] = []
    for part in parts:
        p = (part or "").strip()
        if not p:
            continue
        pl = p.lower()
        # 对 compat/usability：剔除明显安全/审计块（除非同块里也出现本意图关键词）
        if intent in ("compat", "usability"):
            if _has_any(pl, deny_security):
                if intent == "compat" and _has_any(pl, allow_compat):
                    kept.append(part)
                    continue
                if intent == "usability" and _has_any(pl, allow_usability):
                    kept.append(part)
                    continue
                # 直接丢弃该块
                continue
...
def _auto_pick_reference_files_for_target(target_name: str, ref_all_names: List[str]) -> Optional[List[str]]:
    """
    当路由未显式分配 reference_files（=None）时，按目标文件名/目录名做“意图隔离”：
    - 目标是兼容性测试：优先选 compat 参考；排除明显的 security/usability 等参考
    - 其他意图同理
    返回 None 表示不做过滤（仍用全部参考）。
    """
    intent = _infer_intent_key(target_name)
    if intent == "other":
        return None

    def _is_intent(name: str, k: str) -> bool:
        return _infer_intent_key(name) == k

    picked: List[str] = []
    for rn in (ref_all_names or []):
        if not (rn or "").strip():
            continue
        # 若参考本身带有明确“其他意图”，则仅在意图匹配时纳入；否则视作通用参考可纳入
        r_intent = _infer_intent_key(rn)
        if r_intent == "other":
            picked.append(rn)
            continue
        if r_intent == intent:
    ...
    # 如果被筛得太少（只有通用参考），允许再追加“与目标同名/同目录关键词”弱匹配的参考
    if picked and len(picked) <= 2:
        tl = (target_name or "").lower()
        for rn in (ref_all_names or []):
            if rn in picked:
                continue
            rl = (rn or "").lower()
            if not rl.strip():
                continue
            if intent == "compat" and any(k in rl for k in ("兼容", "compat")):
                picked.append(rn)
            elif intent == "usability" and any(k in rl for k in ("可用性", "usability")):
                picked.append(rn)
            elif intent == "security" and any(k in rl for k in ("安全", "security", "cyber")):
                picked.append(rn)
            elif intent == "performance" and any(k in rl for k in ("性能", "performance", "stress", "load")):
                picked.append(rn)
            elif intent == "fault" and any(k in rl for k in ("容错", "tolerance", "fault", "robust", "异常")):
                picked.append(rn)
            elif intent == "data" and any(k in rl for k in ("正确性", "correctness", "accuracy", "data")):
                picked.append(rn)

    # 最终若筛选后没有任何参考，则返回空列表表示“不使用参考摘要”
        ...
        if len(base_manifest) > 1:
            ...
            for bp, bn in base_manifest:
                ...
                try:
                    ...
                    pass
                btok = _intent_tokens(bn)
                sc += min(8, len(ttok & btok) * 2)
                if sc > best_sc:
                    best_sc = sc
                    best = (bp, bn)
            if best is not None and best_sc >= 2:
                picked = best
            else:
                picked = base_manifest[0]
        assigns.append(
            {
                ...
                "_base_path": picked[0],
                "reference_files": None,
                "reason": "路由降级：按文件名/文件夹名意图词粗匹配或默认第一份基础文件；全量参考。",
            }
        )
...
def _plan_multi_base_route_llm(
    *,
    ...
    prompt = (
        ...
        + "\n\n【规则】\n"
        "0) 多份基础文件/参考文件可能来自同一压缩包的不同子目录；name 里可能包含 `压缩包名/子目录/文件名`。你必须把**子目录**当作强信号，用它区分“同模板不同意图”的输出（例如兼容性测试/容错测试/数据正确性测试/可用性测试等）。\n"
...
def _build_srs_traceability_rule_block(template_file_name: str) -> str:
    """
    项目全局硬约束：
    - 需求唯一来源：软件需求规范（SRS）
    - 其他文档（风险/网安风险/架构/详细设计/测试）随 SRS 增量补齐，并追溯到 SRS 需求 ID
    - 根据输出文件名/目录名自动收敛“本文件应覆盖的需求范围”，避免串写
    """
    name = (template_file_name or "").strip()
    nl = name.lower()

    def _has(*kws: str) -> bool:
        return any(k and (k.lower() in nl) for k in kws)

    is_srs = _has("srs", "software requirement", "requirement specification", "软件需求规范", "需求规范", "需求规格")
    is_risk = _has("risk", "风险", "ras", "rmp", "risk management", "risk analysis")
    is_cyber = _has("cyber", "security", "网络安全", "网安", "cybersecurity")
    is_arch = _has("architecture", "架构", "ads", "架构设计", "software architecture")
    is_dd = _has("detailed design", "详细设计", "dds", "software detailed design")
    is_test = _has("test", "测试", "sts", "str", "itr", "execution", "case") or _has("compat", "兼容", "usability", "可用性", "容错", "fault", "correctness", "正确性", "interoperability", "互操作")

    block = (
        "【全局硬约束：SRS 作为唯一需求源（自动注入）】\n"
        "- 本项目所有新增/变更需求只能写入《软件需求规范 / SRS》，并分配/沿用 SRS 需求 ID（如 SRS_XXX、REQ-XXX 等，以基底为准）。\n"
        "- 风险分析、网络安全风险分析、软件架构规范、软件详细设计、各类测试文档：只能“跟随 SRS”补充对应内容；不得在这些文档里新建需求编号或引入不在 SRS 的新需求。\n"
        "- 所有文档的追溯关系以 SRS 需求 ID 为主干：风险/网安风险/架构/设计/测试的条目必须能回指到 SRS 中同一 ID 的需求描述。\n"
    )

    if is_test and not is_srs:
        block += (
            "- 【测试文档范围收敛】本文件只覆盖与其测试类别直接相关的 SRS 需求 ID；不得把其它类别需求写入本文件（例如审计/网络安全需求不得写入互操作性测试）。\n"
            "- 【测试文档追溯】用例/步骤/预期结果必须指向所覆盖的 SRS 需求 ID（或该需求的可引用原文短语）；不允许“无追溯”的新增测试点。\n"
        )
    if is_cyber and is_test:
        block += "- 【网络安全类测试】覆盖 SRS 中与安全/审计/权限/日志/加密相关的需求 ID；不要覆盖互操作性/可用性等无关需求。\n"
    if is_risk and not is_srs:
        block += "- 【风险类文档】风险/控制措施/验证活动必须能回指到 SRS 需求 ID（以及风险文档内部编号，如 CS/风险 ID 以基底为准）。\n"
    if is_arch and not is_srs:
        block += "- 【架构文档】架构模块/接口说明应可追溯到 SRS 需求 ID；不在 SRS 的功能不写入。\n"
    if is_dd and not is_srs:
        block += "- 【详细设计】设计条目应可追溯到 SRS 需求 ID；不在 SRS 的功能不写入。\n"
    if is_srs:
        block += "- 【SRS】本次若出现新增/变更需求，必须考虑其对风险/架构/详细设计/测试的影响点（在其他文档中以追溯方式体现；不要在 SRS 外新增需求）。\n"
...
_BASE_FORMAT_MERGE_PRINCIPLE_BLOCK = (
    "【基础文件 vs 参考文件（核心原则，必读）】\n"
    "- **复用基础文件的是「格式与版式」**：章节标题层级、表格表头与列结构、编号样式、段落/单元格版式以基础/EXISTING_DRAFT_TEXT 为准。\n"
    "- **不是冻结基础文件的正文**：参考文件（INPUT_DOCS_EXCERPT）相对基础**多出的**表格行、段落、条目、用例、需求、定义、法规分项等，**必须**合并进基础文档最接近的已有章节/表格。\n"
    "- **明确允许**：在已有表格中增加新行（insert_table_row_after_contains）；在已有章节内增加新段落/列表项（insert_paragraph_after_contains）；对重复但表述不一致处修订（replace_*）。\n"
    "- **禁止误解**：不得因「保持基础格式」而只改少量单元格就结束；参考中有多条新增用例/多段新条文时，应批量插入对应行数，**不得漏合并**。\n"
    "- **边界**：不得新增顶层章节标题；编号沿用基底规则（无法确定用 TBD）；不得编造追溯编号；项目标识以 NEW_BASIC_INFO 为准。\n"
)
...
1. 你必须以下面的"基底文本（TEMPLATE_TEXT）"为格式基础：保留其标题/章节层级/表格行列/编号与表头字段，尽可能原样保留与本次修改无关的部分。
   ...
   - 标题、章节层级、版式样式（含表格列结构/表头）必须以基底为准；参考文件不得改变基底既有文档骨架。
2. **参考文件合并规则（强制）**：
   - 参考文件比基底多出的有效内容：补充到基底最接近的已有章节/表格中。
   - 参考文件与基底存在重复主题且内容有差异：优先在基底原段落/原单元格做修订更新，不得在相邻位置重复新增一段近似内容。
   - 参考与基底重复且一致：保持基底原文，不做机械改写。
3. 默认允许修改项（**格式限基底，内容须合并参考**）：
   - 基本信息字段：项目名称、产品名称、型号规格、注册单元名称（出现处需与 NEW_BASIC_INFO 一致）
   - **凡 INPUT_DOCS_EXCERPT 与基底文档主题可对应的章节、表格、定义、需求、风险、设计、测试用例、法规分项等，均应更新**（不仅限于「系统功能」字样章节）；参考多出的行/段/条必须写入基底对应落点
   - 若基底文本中存在明显缺失章节（对照 TEMPLATE_REFERENCE_TEXT 可识别）：允许在**已有大节内**补充段落或表格行，但不得新增顶层章节标题与项目范围外内容
4. **重要——本文件专用 Skills 扩展修改范围**：若上方硬约束中包含「本文件专用 Skills」，则其列举的每一条操作指令（如"变更记录追加""目录更新""预期用途替换""版本号升级"等）均属于本次允许修改的范围，你必须逐条执行，不得因默认限制而跳过。本文件专用 Skills 的优先级高于第 3 条默认限制。
5. **重要——参考文件仅作技术内容来源**：INPUT_DOCS_EXCERPT 用于对齐**技术类**表述（定义与缩略语、法规/标准条目、风险与控制措施、权限/日志/流程等功能性描述等）。其中若出现其它案例项目的**公司名称、产品名称、型号规格、注册信息**等标识，**不得**当作本项目信息使用；项目标识一律以【项目约束】与 **NEW_BASIC_INFO** 为准。你必须仔细阅读参考内容，将其中与基底文档相关章节对应的技术要点逐项更新到对应位置，不能只更新部分要点而遗漏其余。
6. **重要——全自动适配“落点”（不要反问用户放哪）**：
   ...
   - 在目标章节内：你必须对比参考与基底，**自动识别**差异类型（新增 / 细化或替换 / 删除或废止表述），再按下面处理：
     ...
     - 若参考有要求而基底完全缺失：在该章节**新增**条目或段落（沿用该章节编号规则；无法确定编号则用 TBD，禁止编造追溯编号）。同一小节内多条缩略语定义、多条 `[n]` 参考文献或多条法规/标准：应**分项换行**书写，沿用该小节既有字体层级与编号样式，不要合并为单一大段。
...
8. **重要——同一项目多份文档互相关联**：
   ...
   - 发生冲突时：**公司名称/产品标识/型号规格/注册单元/项目代码**等项目身份信息必须以 **NEW_BASIC_INFO** 与【项目约束】为准，**禁止**用参考文件中的旧案例信息覆盖。若冲突属于**技术口径**（模块清单、权限规则、日志字段、定义表述等），可在允许修改范围内以 INPUT_DOCS_EXCERPT / INPUT_SYSTEM_FUNCTIONALITY 为准，并在多份文档间同步，避免“某文档新增模块但另一文档仍缺失”。
9. 法规/编号/追溯 ID：除非模板本身对应位置必须变化，否则保持与模板一致；任何出现的 URS/REQ/CS/CE/HC 等编号必须逐字复制，不得改写/补零/拼接。
10. 若输入文档未提供足够证据：请不要编造；在允许修改的位置保留模板原文或写入 TBD（不得虚构编号与证据来源）。
11. 输出要求：只输出"替换后的完整文档文本"，不要输出 JSON、不要输出解释、不要输出 markdown 代码块标记（不要用 ```）。
12. **语言一致性（必须遵守）**：若【文档语言】为英文（English / en / 英文版），则你输出的全文（包括新增段落、表格内容、图注/说明、修订/变更描述等）必须为英文；即使 INPUT_DOCS_EXCERPT 为中文，也只能将其信息翻译/转写为英文后写入，不得夹带中文。
...
【NEW_BASIC_INFO（全局/用户选择的项目信息；与参考文件冲突时以此为准）】
{new_basic_info}
...
【任务目标】
- 你需要对照 INPUT_DOCS_EXCERPT（及 INPUT_SYSTEM_FUNCTIONALITY），系统比较参考与 EXISTING_DRAFT_TEXT 的差异，对**所有可对应主题**的章节、表格、定义、需求、用例、风险条目等生成修改（不仅限于「系统功能」章节）。**项目名称/产品名称/型号/注册单元**等基本信息字段的合法取值以 **NEW_BASIC_INFO** 与【项目约束】为准，不得因参考文件中的案例标识而改写。

【枚举优先（强制——避免"漏合并一大半"）】
- 在生成任何 operation 之前，你必须先从 INPUT_DOCS_EXCERPT 中**穷举**与本目标文档主题对应的"枚举型实体"，例如：
  - 兼容性测试用例表：所有出现的**设备型号/规格清单**（如 YH-380、YH-690F、YH-820Auto、YH-825ST、YH-830LAB、YH-920STI 等）。
  - 需求/追溯表：所有 URS/REQ/SRS/CS/CE/HC 等编号。
  - 风险分析表：所有风险编号 / 危险源条目。
- 然后**对照基底**（EXISTING_DRAFT_TEXT），识别其中**已经存在**与**尚未覆盖**的实体。
- 你必须在 PATCH_JSON 顶层输出一个 `entities_inventory` 对象（仅用于审计与覆盖率回扫，不参与文档执行），结构如下：
  - `entity_kind`: 字符串，描述本目标的实体类型（如 `device_model` / `srs_id` / `risk_id`）。
  - `from_reference`: 数组，参考中出现的全部实体（**逐字复制原文**，不要改写大小写或补零）。
  - `present_in_base`: 数组，基底中已经覆盖到的实体。
  - `missing_in_base`: 数组，基底中**缺失**、应通过本次 operation 补齐的实体。
- 对 `missing_in_base` 中的**每一个实体**，你必须生成**对应的 operation**（通常是 `insert_table_row_after_contains`，或在涉及多列时配合 `row_by_header`）；不得"挑几个写完就停"。
- 若参考与基底主题不匹配（例如本目标只覆盖某一类设备而参考混杂多类），允许在 `entities_inventory.note` 字段简要说明范围，但仍必须对**确属本目标范围**的实体逐项覆盖。
- 标题、章节层级、版式风格、表格列结构必须以 EXISTING_DRAFT_TEXT（基础文档）为准；参考文件仅用于补充新增信息和修订更新重复内容，不得改造基底文档骨架。
- 参考文件比基础文档多出的有效内容，应补充到基础文档已有章节/表格的最接近落点；若参考与基础存在重复主题但内容不一致，优先 replace 原段落/原单元格进行修订更新，禁止插入两段语义重复内容。
...
- 你必须对比参考与基础，**自动识别**需 **新增 / 细化或替换 / 删除** 的位置，再生成 operation：
  ...
  - **参考表格行数多于基础时（强制）**：对测试用例表、需求表、追溯表等，须用 `insert_table_row_after_contains` 把参考中多出的每一行（或批量多行 `new_text` 换行分隔）写入基底同表；**禁止**只改一两格就结束。
  - 若参考表明条目应移除或废止：在可锚定前提下使用 **delete** 或替换为符合参考的表述。
  - **完成前自检**：对照 INPUT_DOCS_EXCERPT，参考中已出现而基础中缺失的条目/行/段，在可锚定前提下均应有 operation；不得以「保持基础格式」为由跳过合并。
  - **避免“叠两段几乎一样的正文”**：当 EXISTING_DRAFT_TEXT 中已存在可承载该信息的段落/单元格（即使措辞略不同但语义与信息高度重叠），必须优先使用 `replace_paragraph_contains` / `replace_table_cell_contains` **改写原位置**；只有在确实缺失、找不到可替换锚点时才使用 `insert_paragraph_after_contains`。
  - **不要把变更记录写进封面签字/审批表**：若基础文档存在“变更/修订记录/Revision history”等章节表格，应优先用表格类 operation 写入该表；不要把变更记录写成普通段落落到封面附近。
  - **项目标识与参考文件**：INPUT_DOCS_EXCERPT 仅作**技术内容**参考；其中出现的其它案例**公司名/产品名/型号/注册信息**不得当作本项目信息。项目标识一律以 **NEW_BASIC_INFO** 与【项目约束】为准。
  - **多行 new_text（定义/文献/法规列表）**：多条缩略语定义、多条 `[n]` 参考文献、或以分号分隔的多条标准/法规，应在 `new_text` 中用**换行**分成多行（或保持分号分隔的长串由程序再拆）；程序导出时会展开为多段，并继承锚点段落的**字体/字号/首行缩进/行距**，且在 References/参考文献类章节对 `[n]` **续号**。
...
- 若参考文件要求的是“新增信息”，而基础文档对应章节已存在标题/小节且需要在原章节中补充内容：
  - 优先判断该小节是否已有“同类信息段落”可改写：有则 `replace_paragraph_contains`；没有再用 `insert_paragraph_after_contains`：以该章节内已有段落或小节标题（EXISTING_DRAFT_TEXT 中原文）作为 anchor，把新内容插入其后（不得新增新标题）。
  - 若需要补充到原表格中（新增行）：可使用 `insert_table_row_after_contains`，以表头或某行关键单元格原文作为 anchor，在其后插入新行。**禁止**把表格多行内容（含 GNxx-数字、SRS_xxx、制表符分列）用 `insert_paragraph_after_contains` 写入正文——否则 Word 会出现可见的 `\\t`/`\\n` 乱码且破坏版式。**多条用例可写在同一 operation**：`new_text` 内用**真实换行**分隔多行，每行用**真实制表符**分列（JSON 中写 `\\t`/`\\n` 即可，勿写成可见的反斜杠+t）；程序会按序逐行插入表格。 **推荐按表头对齐（强制，≥3 列表格）**：在同一条 operation 中提供 **row_by_header**（对象：键为与表头单元格相同或可部分匹配的列名，值为该列文本），或将 **new_text 设为 JSON 对象**（键值同上）；程序会在锚点行上方自动识别表头行并把各值写入对应列（**docx / xlsx 均支持**）。**无 row_by_header 且无 \\t 分列时，程序会拒绝写入多列表格**，以防整段挤入首列、与表头错列。仍兼容用制表符 `\\t` 按**从左到右**分列：`new_text` 中 **段数须等于表头列数**（缺列用空段占位 `\\t\\t`）；**禁止**把多列不加 `\\t` 拼成一串。
  - **活动/人员/时间、用例编号/步骤/预期结果等多列表**：必须 **row_by_header** 或 **\\t 分列且段数=列数**；「人员」列只写姓名，「活动」列只写活动描述，不得把用例长描述塞进人员列；**禁止**用 `replace_table_cell_contains` 整格覆盖仍正确的姓名/编号单元格。
  - **Excel 单元格替换（字段名 vs 值）**：`replace_table_cell_contains` 必须写入**放值的单元格**，不要只命中左侧「字段名/标签」单元格。请同时提供 **`sheet`（工作表名）** 与 **`cell`（如 D6）** 指向值格；`anchor` 仍填来自 EXISTING_DRAFT_TEXT 的短语用于核对。可选 **`before`**：若值格内仅需替换其中一段子串，则 `before` 为被替换子串，`new_text` 为替换后完整单元格文本或仅新子串（与程序实现一致：子串存在则做子串替换，否则整格写入 `new_text`）。
  - **测试用例/追溯表（程序侧已处理，模型只需给行内容）**：首列用例编号形如 `GN3-21`、`GN3-250`（前缀-数字）时，导出程序会扫描**整张表**内同前缀的最大编号；**新插入行**的首列会自动设为「最大编号 +1」。若首列编号**已在表中出现**：整行相似度≥90% 则原地更新，否则插入新行。**列数必须与表内数据行一致**：优先用 **row_by_header**（键名抄表头原文）；或用 `\\t` 分列且**段数=表头列数**（缺列用空段占位，如 `GN71-274\\tSRS_009\\t兼容性测试\\t…\\t\\t`），禁止只给 3～4 段却期望写入 10 列。程序会克隆锚点数据行的表格结构，列数不足会导致竖线错位。
...
【允许修改项】
1) 默认范围（**格式限基底，内容须合并参考**）：
   - 基本信息字段：项目名称、产品名称、型号规格、注册单元名称
   - **凡参考与基底可对应的正文、表格、定义、需求、用例、风险、法规分项等，均应生成 operation**；参考比基底多的表格行/段落/条目必须用 insert 类操作写入，不得漏合并
   - 法规/编号/追溯 ID：须逐字复制已有编号；新增行编号沿用表内/章节内规则（程序可自动续号）
...
3) **充分利用参考文件的技术内容**：INPUT_DOCS_EXCERPT 用于提取与基底相关的技术要点（权限列表、日志规则、功能描述、定义与法规条目等）并生成 operation；**不得**用参考文件中的案例项目标识替换 NEW_BASIC_INFO 中的本项目信息。你必须逐份阅读并逐项覆盖，不能只更新部分要点而遗漏其余。
...
- 每个 operation 必须包含：
  ...
  - anchor: 用于定位的锚点短语（须来自 EXISTING_DRAFT_TEXT 的原文片段；**Excel** 的 `replace_table_cell_contains` 若已提供 `sheet`+`cell` 则可留空 anchor，否则仍必填）
  - sheet / cell（可选，**强烈建议 Excel 替换值时使用**）：`sheet` 为工作表名（如 `Cover`），`cell` 为单元格地址（如 `D6`），用于精确定位**值单元格**。
  - before（可选，Excel/Word）：单元格内要被替换的子串；与 `new_text` 配合可做局部替换。
  - new_text: 替换后的文本（可多行）。多条缩略语定义、多条 `[n]` 文献、多条法规/标准：用**换行**分隔；程序会展开为多段并继承锚点格式，在 References 类章节对 `[n]` 续号。表格插行时也可把 **new_text 设为 JSON 对象**（列名→单元格值），与 **row_by_header** 等价。
  - row_by_header（可选）：对象，`insert_table_row_after_contains` 时用于**按表头列名**对齐写入各列；键名尽量抄 EXISTING_DRAFT_TEXT 中该行上方表头原文。
 ...
 - **表格多行单元格（测试步骤/预期结果等）强制规则**：若你用 `replace_table_cell_contains` 修改的单元格在基底里是**多行**（单元格内包含换行，常见于“操作步骤/预期结果”列），则：
   - 你输出的 `new_text` 必须为**完整英文多行**，逐行翻译并**保留换行结构**（不能只翻译首行，否则会覆盖并丢失同格其它行）。
   - 仅当 anchor 能唯一命中该**整格文本**时才允许 `require_unique=false`；若 anchor 只是该单元格的首行短语，必须改用更长的 anchor（覆盖整格特征）或提供 `before` 做子串替换，避免误伤。
...
# --- 实体覆盖回扫（用于在 patch 解析成功后判断是否漏合并大量参考实体） ---

# 实体类型 → 模板文件名关键词（小写匹配）。命中任一关键词即启用对应正则。
_ENTITY_INTENT_KEYWORDS: Dict[str, Tuple[str, ...]] = {
    "device_model": ("兼容", "compatibility", "compat"),
    "srs_id": ("追溯", "rtm", "traceability", "可追溯"),
    "risk_id": ("风险", "risk", "hazard"),
}

# 每种实体类型的候选正则；多个正则会取并集。
_ENTITY_PATTERN_LIBRARY: Dict[str, Tuple[re.Pattern, ...]] = {
    "device_model": (
        # YH-820Auto / YH-825ST / SH-100 / ABC-12 / ABC-12A 等"字母前缀+数字+可选字母后缀"
        re.compile(r"\b[A-Z]{1,4}-\s?\d{2,4}[A-Z]{0,6}\b"),
    ),
    "srs_id": (
        re.compile(r"\b(?:SRS|REQ|URS|CS|CE|HC|FRS)[\s_-]?\d{1,5}(?:[._-]\d{1,3})?\b", re.IGNORECASE),
    ),
    "risk_id": (
        re.compile(r"\b(?:HAZ|RID|RISK|HZ)[\s_-]?\d{1,5}\b", re.IGNORECASE),
    ),
}


def _entity_kind_for_target(template_file_name: str) -> Optional[str]:
    name = (template_file_name or "").lower()
    for kind, kws in _ENTITY_INTENT_KEYWORDS.items():
        if any(k in name for k in kws):
            return kind
    return None


def _extract_entities_by_kind(text: str, kind: str) -> set:
    pats = _ENTITY_PATTERN_LIBRARY.get(kind) or ()
...
def _entities_covered_in_patch_json(patch_json: str, kind: str) -> set:
    """从 patch 的 anchor / new_text / row_by_header 文本中抽取已覆盖的实体集合。"""
    try:
    ...
    for op in (obj.get("operations") or []):
        if not isinstance(op, dict):
            continue
        for k in ("anchor", "new_text", "before"):
            v = op.get(k)
            if isinstance(v, str):
                texts.append(v)
        rbh = op.get("row_by_header")
        if isinstance(rbh, dict):
            for v in rbh.values():
                if isinstance(v, str):
                    texts.append(v)
    # entities_inventory 中模型自报的"已覆盖"实体也算（但仅作辅助）
    inv = obj.get("entities_inventory")
    if isinstance(inv, dict):
        for k in ("present_in_base", "from_reference"):
            arr = inv.get(k)
            if isinstance(arr, list):
                for x in arr:
                    if isinstance(x, str):
                        texts.append(x)
    return _extract_entities_by_kind("\n".join(texts), kind)


def _merge_patch_operations(primary_json: str, supplementary_json: str) -> str:
    """把补全调用产出的 operations 合并到主 patch 顶层 operations 末尾。仅合并 operations。"""
    try:
        a = json.loads(primary_json or "{}")
        b = json.loads(supplementary_json or "{}")
    except Exception:
        return primary_json
    if not isinstance(a, dict):
        return primary_json
    if not isinstance(b, dict):
        return primary_json
    ops_a = a.get("operations") if isinstance(a.get("operations"), list) else []
    ops_b = b.get("operations") if isinstance(b.get("operations"), list) else []
    if not ops_b:
        return primary_json
    a["operations"] = list(ops_a) + list(ops_b)
    # 合并 entities_inventory.note 仅作可读补丁说明
    inv_a = a.get("entities_inventory") if isinstance(a.get("entities_inventory"), dict) else None
    if inv_a is not None:
        prev_note = str(inv_a.get("note") or "").strip()
        new_note = (prev_note + ("; " if prev_note else "") + "已通过补全调用追加缺失实体的 operations").strip()
        inv_a["note"] = new_note
        a["entities_inventory"] = inv_a
...
class DocumentDraftGenerator:
    ...
    def _gen_one_file(
        ...
        *,
        ...
        strategy_block = (
            "【生成策略】\n"
            + (
                "- change（注册变更）：以基础文件为**格式底稿**（标题层级、表头列结构、版式），对照参考（INPUT_DOCS_EXCERPT、INPUT_SYSTEM_FUNCTIONALITY 等）**逐项合并**差异：参考多出的表格行/段落/条目必须**新增**到基底对应章节；重复不一致处**修订**；一致处保留。不得因「保持格式」而几乎不改内容。修改须可定位、可审计，避免无依据的全文替换。\n"
                if strat == "change"
        ...
        # 项目全局硬约束：SRS 唯一需求源 + 按文件名/目录名自动收敛范围（所有身份生效）
        qa_extra_block = (qa_extra_block + "\n" + _build_srs_traceability_rule_block(template_file_name)).strip() + "\n"
        prompt = tmpl.format(
            ...
            base_format_merge_block=_BASE_FORMAT_MERGE_PRINCIPLE_BLOCK,
        ...
        if _audit_txt:
            ...
            if inplace_patch:
                prompt += (
                    ...
                    "**风险分析/联合使用等多列表（≥5 列）**：`insert_table_row_after_contains` 必须使用 **row_by_header**"
                    "（键名抄表头原文，如 Risk ID、危险(源)、严重程度、出现概率、风险区域、措施、风险需求 ID 等），"
                    "禁止把整行描述或「1/2/I」等数值塞进 Risk ID 列；"
                    "**Risk ID 列只写编号（如 JC01、3），严重程度/概率/区域必须写入各自列**；"
        ...
        if _uap:
            ...
            if len(_uap) > _lim:
                _uap = _uap[:_lim] + "\n【提示】用户追加指令过长已截断。"
            prompt += (
                "\n\n【用户追加指令（须与上文硬约束一并遵守；不得据此编造追溯编号或超出适用范围）】\n"
                + _uap
            )
    ...
    def generate(
        ...
        *,
        ...
        # 参考文件已在项目向量库中重名时：skip=不重复向量化；overwrite=删除后重新向量化
        input_vector_on_duplicate: str = "skip",
        # 集成 API：用户自带 LLM 凭据（禁止持久化）
        client_llm: Optional[ClientLlmConfig] = None,
        # 每文件生成提示词末尾追加（aiword 等）；与 skills/rules 无关，不写入案例补丁表
        user_prompt_append: str = "",
    ) -> GeneratedCaseDocs:
        ...
        with tempfile.TemporaryDirectory(prefix="aicheckword_draft_") as td:
                ...
                def _build_combined_input_excerpt() -> str:
                    ...
                    for fp, _fn in saved_inputs:
                        try:
                            ...
                            # 单文件摘要上限放大：原 18000 不足以容纳长用例表/追溯表中所有型号/编号，
                            # 导致后续 patch 生成时模型只能看到部分实体而漏合并。
                            per_file_cap = 36000
            ...
            # 4) 训练输入文件到项目专属向量库（可跳过；重名时可覆盖或沿用已有向量）
            _vec_train_meta: Dict[str, Any] = {}
            if saved_inputs and not skip_input_vector_training:
                from src.core.draft_input_vectorization import train_input_files_to_project

                _progress(f"训练输入文件（向量化，共 {len(saved_inputs)} 个）…", 0.10)
                n_in = len(saved_inputs)

                def _embed_factory(i_in: int, n: int, fn: str):
                    frac_lo, frac_hi = 0.10, 0.22
                    base_frac = frac_lo + (frac_hi - frac_lo) * ((i_in - 1) / max(1, n))
                    span = (frac_hi - frac_lo) / max(1, n)
                    ...
                    def _embed_cb(done: int, total: int) -> None:
                        ...
                        _progress(
                            f"训练输入文件 ({i_in}/{n_in})：{fn} · 向量化 {d}/{t} 块",
                            min(sub, base_frac + span * 0.99),
                        )

                    return _embed_cb

                _vec_train_meta = train_input_files_to_project(
                    self.agent,
                    int(project_id),
                    saved_inputs,
                    on_duplicate=(input_vector_on_duplicate or "skip"),
                    progress=_progress,
                    embed_callback_factory=_embed_factory,
                )
            ...
            if saved_inputs:
                ...
                _use_inline_basic = bool(skip_input_vector_training)
                if not _use_inline_basic and _vec_train_meta.get("skipped"):
                    # 重名跳过向量化时仍用本次文件摘录提取基本信息，避免仅依赖旧库摘要
                    _use_inline_basic = True
                extracted_basic_info_text = self.agent.extract_and_save_project_basic_info(
                    int(project_id),
                    provider=provider,
                    client_llm=client_llm,
                    inline_source_text=combined_input_text if _use_inline_basic else None,
                )
            else:
            ...
            # 原 42000 在长用例表/追溯表场景下过小，导致参考实体（型号/编号清单）被截断遗漏。
            if len(input_excerpt) > 90000:
                input_excerpt = input_excerpt[:90000] + "\n【提示】输入/参考文件摘要已截断。"
            ...
            if multi_base_auto_route and (base_files_manifest or []):
                ...
                # base_files_manifest 但未启用自动路由：按文件名映射 base_path（Base-only 常见）
                if base_files_manifest and not route_map:
                    for pth, name in (base_files_manifest or []):
                        if name and pth:
                            route_map[str(name)] = {"base_path": str(pth), "reference_files": None}

            # 未匹配到任何目标的基础文件：以基础文件名自身作为目标加入生成队列，避免“上传了文件却没有产出”
            leftover_base_only_targets: set = set()
            try:
                _used_base_paths = {str((v or {}).get("base_path") or "") for v in (route_map or {}).values() if v}
                _used_base_paths.discard("")
                _leftover: List[Tuple[str, str]] = []
                if base_files_manifest:
                    for pth, name in (base_files_manifest or []):
                        if not pth or not name:
                            continue
                        if str(pth) in _used_base_paths:
                            continue
                        _leftover.append((str(pth), str(name)))
                if existing_base_files:
                    for tf, pth in (existing_base_files or {}).items():
                        if not pth or not tf:
                            continue
                        if str(pth) in _used_base_paths:
                            continue
                        if tf in route_map and (route_map[tf] or {}).get("base_path"):
                            continue
                        _leftover.append((str(pth), str(tf)))
                if _leftover:
                    _existing_targets = set(chosen)
                    for pth, name in _leftover:
                        target = name
                        if target in _existing_targets and (route_map.get(target) or {}).get("base_path") != pth:
                            k = 2
                            stem = Path(name).stem or name
                            suf = Path(name).suffix
                            while True:
                                cand = f"{stem}#{k}{suf}"
                                if cand not in _existing_targets:
                                    target = cand
                                    break
                                k += 1
                        chosen.append(target)
                        _existing_targets.add(target)
                        route_map[target] = {"base_path": pth, "reference_files": None}
                        leftover_base_only_targets.add(target)
            except Exception:
                leftover_base_only_targets = set()
            ...
            for template_file_name in chosen:
                ...
                rows = []
                template_text = ""
                _is_leftover_only = template_file_name in (leftover_base_only_targets or set())
                if (not base_only_mode) and (not _is_leftover_only):
                ...
                # base_only_mode 或未匹配模板的基础文件：不依赖模板案例文本；模板文本由 existing_text 兜底（见下方）
                ...
                if _ref_names is None:
                    # 路由未显式分配参考子集：按目标文件名/目录名做“意图隔离”兜底，
                    # 避免如“兼容性测试”误引入“网络安全”等不相关参考。
                    try:
                        _ref_auto = _auto_pick_reference_files_for_target(
                            template_file_name, [rn for _rp, rn in (saved_inputs or [])]
                        )
                    except Exception:
                        _ref_auto = None
                    if _ref_auto is None:
                        input_excerpt_this = _filter_excerpt_by_intent(input_excerpt, target_name=template_file_name)
                    else:
                        _nl = [x for x in (_ref_auto or []) if (x or "").strip()]
                        _hint = (
                            f"【输入/参考文件清单（自动意图隔离子集）】共 {len(_nl)} 个：{', '.join(_nl)}\n\n"
                            if _nl
                            else "【输入/参考文件清单（自动意图隔离子集）】无\n\n"
                        )
                        input_excerpt_this = (_hint + _filter_combined_input_by_reference_names(combined_input_text, _ref_auto)).strip()
                        input_excerpt_this = _filter_excerpt_by_intent(input_excerpt_this, target_name=template_file_name)
                else:
                    ...
                    input_excerpt_this = _filter_excerpt_by_intent(input_excerpt_this, target_name=template_file_name)
                    if len(input_excerpt_this) > 90000:
                        input_excerpt_this = input_excerpt_this[:90000] + "\n【提示】输入/参考文件摘要已截断。"
                ...
                if bool(inplace_patch and bool(existing_text)):
                    ...
                    if not patch_json.strip():
                        # 关键：避免 export_like_base 走“附录追加”导致格式破坏。
                        # 在就地修改模式下，优先做一次重试，强制模型输出可解析 PATCH_JSON。
                        try:
                            raw_out2, _had2 = self._gen_one_file(
                                base_case_id=int(base_case_id),
                                template_file_name=template_file_name,
                                template_text=(existing_text or template_text),
                                existing_draft_text=existing_text or "",
                                ...
                                input_system_functionality=sys_fun_text,
                                input_docs_excerpt=(
                                    "【强制重试】你刚才未输出可解析的 PATCH_JSON。你必须严格按格式输出："
                                    "先输出 `### PATCH_JSON` 后跟一个 JSON 对象（含 operations 数组），"
                                    "再输出 `### UPDATED_TEXT`。不得省略 PATCH_JSON；不得用 markdown 代码块。\n\n"
                                    + (input_excerpt_this or "")
                                ),
                                draft_strategy=draft_strategy,
                                author_role=((author_role_map or {}).get(template_file_name) or author_role or ""),
                                inplace_patch=True,
                            ...
                            if pj2.strip():
                                patch_json, new_text = pj2, nt2
                        except Exception:
                            pass
                    if patch_json.strip():
                        # 实体覆盖回扫：参考文件中包含可枚举实体（兼容性测试型号/追溯编号/风险编号）时，
                        # 比对 patch 已覆盖集合，若覆盖率 < 80% 则触发一次"补全调用"。
                        try:
                            _kind = _entity_kind_for_target(template_file_name)
                            if _kind:
                                ref_entities = _extract_entities_by_kind(
                                    input_excerpt_this or "", _kind
                                )
                                base_entities = _extract_entities_by_kind(
                                    existing_text or "", _kind
                                )
                                covered_now = _entities_covered_in_patch_json(patch_json, _kind)
                                # 应被本次新增覆盖的实体 = 参考中出现且基底中不存在
                                expected_to_add = (ref_entities - base_entities)
                                if expected_to_add:
                                    missing = sorted(x for x in expected_to_add if x not in covered_now and x not in base_entities)
                                    total = max(1, len(expected_to_add))
                                    coverage = (total - len(missing)) / total
                                    if coverage < 0.8 and missing:
                                        _missing_preview = "、".join(missing[:80])
                                        _retry_hint = (
                                            "【补全模式 · 实体未覆盖】\n"
                                            "你刚才输出的 PATCH_JSON 漏掉了下列在参考中出现而基底中缺失的实体，请仅"
                                            f"为这些实体生成**新增 operation**（保持现有 operation 不变；只追加）。\n"
                                            f"实体类型：{_kind}\n"
                                            f"未覆盖实体（共 {len(missing)} 个，已截前 80）：{_missing_preview}\n"
                                            "要求：\n"
                                            "1. 每个未覆盖实体至少对应一条 operation；测试用例表用 insert_table_row_after_contains，"
                                            "可在同一 operation 用换行/制表符批量插入多行；\n"
                                            "2. 锚点 anchor 必须来自 EXISTING_DRAFT_TEXT 原文（建议用最近一条已存在的实体行作锚点）；\n"
                                            "3. 不要重复已有 operation，不要修改已覆盖实体；\n"
                                            "4. 仍按 `### PATCH_JSON` + `### UPDATED_TEXT` 输出；UPDATED_TEXT 可保留为简短占位（程序仅采纳 PATCH_JSON 中新追加的 operations 合并到主 patch）。\n\n"
                                            + (input_excerpt_this or "")
                                        )
                                        try:
                                            raw_sup, _ = self._gen_one_file(
                                                base_case_id=int(base_case_id),
                                                template_file_name=template_file_name,
                                                template_text=(existing_text or template_text),
                                                existing_draft_text=existing_text or "",
                                                ...
                                                new_basic_info=new_basic_info_for_prompt,
                                                input_system_functionality=sys_fun_text,
                                                input_docs_excerpt=_retry_hint,
                                                draft_strategy=draft_strategy,
                                                author_role=((author_role_map or {}).get(template_file_name) or author_role or ""),
                                                inplace_patch=True,
                                                workspace_root=Path(__file__).resolve().parents[2],
                                            ...
                                            pj_sup, _nt_sup = _parse_patch_and_updated_text(raw_sup)
                                            if pj_sup.strip():
                                                merged = _merge_patch_operations(patch_json, pj_sup)
                        ...
                        try:
                            ...
                            pass
                        patches[template_file_name] = normalize_patch_json_string(
                            patch_json.strip()
                        )
