...
_AUTHOR_ROLE_PROMPTS: Dict[str, str] = {
    ...
    "qa": (
        "【编写身份：测试工程师】\n"
        "- 侧重：可测试性、验收准则、边界与异常、回归关注点；需求表述应能映射到测试条件（在 SRS 等语境下）。\n"
        "- 表述：条件—输入—期望结果式措辞；避免模糊形容词。\n"
        "- 避免：代替研发写代码级设计；不要虚构测试用例编号除非模板已有格式。\n"
    ),
    "cm": (
        "【编写身份：配置管理员】\n"
...
- 若参考文件要求的是“新增信息”，而基础文档对应章节已存在标题/小节且需要在原章节中补充内容：
  ...
  - 若需要补充到原表格中（新增行）：可使用 `insert_table_row_after_contains`，以表头或某行关键单元格原文作为 anchor，在其后插入新行；new_text 用制表符 `\\t` 分隔每列（列数需与原表格一致或更少）。
