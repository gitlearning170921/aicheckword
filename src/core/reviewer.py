...
from .db import get_dimension_options, get_corrections_for_collection
...
- **需求—开发—测试—风险可追溯性（医疗器械软件，优先）**：关注各文档中出现的**追溯标识**（如用户需求 ID/REQ ID、软件需求 ID、风险 ID、风险分析文档中的 **CS 编号**、危害编号、测试用例/报告编号、追溯矩阵中的行/列引用等）是否形成**闭环且语义一致**。重点包括：（1）《软件需求规范》或同等文件中填写的风险 ID、用户需求 ID 与《风险管理/风险分析》文档中对应编号及控制措施描述是否**一致**；（2）《软件可追溯性分析报告》（或矩阵）中填写的「需求—设计—实现—测试—风险」对应关系，与《软件需求规范》及设计/开发/测试文档中的**实际表述**是否一致，是否存在「有 ID 无对应条目」「同一 ID 在不同文档中指向不同内容」「应追溯却断链」；（3）若本批含多份文档，须交叉核对**同一追溯 ID** 在各文档中的**目标内容**（需求文字、风险措施、测试覆盖）是否一致，不一致或断链须作为**一致性/完整性**审核点列出并写明涉及文档与应修正关系。（4）若上下文或知识库提供组织**专门针对可追溯性的受控程序文件**（如《标识与可追溯性控制程序》《软件可追溯性管理程序》等，以实际文件名为准），须审核注册/项目文档中的追溯方式、**ID 编码规则**、矩阵或报告**必备要素**、记录与签批要求等是否**符合该程序文件的规定**；不符合须作为**合规性**或**一致性**审核点列出并引用程序条款。
...
14. **需求—开发—测试—风险可追溯性（医疗器械软件）**：核查文档中的用户需求 ID、软件需求 ID、风险 ID、**CS 编号**（或组织规定的危害/风险控制编号）、测试与追溯矩阵引用是否**跨文档一致**且**可追溯闭环**；《软件可追溯性分析报告》与《软件需求规范》、风险分析、设计/测试文档中对同一 ID 的描述须一致；发现 ID 指向矛盾、断链、重复编号混用或「有号无文」须作为一致性或完整性审核点列出，location 写明具体表格/章节与 ID。
15. **可追溯性与程序文件符合性**：若审核点参考知识或上下文中包含组织**专门针对可追溯性的受控程序文件**（或程序摘录），须审核待审文档（尤其软件需求规范、可追溯性分析报告、设计/测试类记录）中的追溯标识规则、矩阵结构、必备栏目、更新与签批等是否**符合程序规定**；不符合须作为合规性或一致性审核点，**regulation_ref** 可写程序文件名称与条款号，location 写明与程序不符的具体位置。
...
10. **需求—开发—测试—风险可追溯性**：用户需求/软件需求/风险/CS 编号等追溯 ID 须在多文档间一致；《软件可追溯性分析报告》与需求规范、风险分析、测试文档对同一 ID 的指向须一致；断链或矛盾须作为审核点列出。若参考知识含**可追溯性程序文件**，须核对文档追溯做法与程序规定是否一致。
...
- **需求—开发—测试—风险可追溯性（跨文档，优先）**：本批若同时包含《软件需求规范》《风险管理/风险分析》《软件可追溯性分析报告》及设计/开发/测试类文档，须逐项核对：（1）需求规范中记载的**风险 ID、用户需求 ID** 与风险分析文档中的**风险编号、CS 编号**及控制措施**文字是否一致**；（2）可追溯性分析报告中的矩阵或文字描述，与需求规范、设计说明、测试计划/报告中的**同一 ID 所指向内容**是否一致；（3）**同一追溯 ID** 在需求、开发、测试、风险四类文档之间是否形成闭环，是否存在有引用无对应条目、编号混用或指向矛盾。不一致、断链须作为一致性/完整性审核点，**location** 须写明文档名称、表格或章节及具体 ID。（4）若本批含组织**可追溯性管理程序文件**（或程序摘录），须核对项目/注册文档中的追溯标识、矩阵必备项、记录与签批等是否**符合程序条款**；不符合须作为合规性审核点并写明程序条款与文档位置。
