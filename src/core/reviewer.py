...
- **编审批人员逻辑**：编制、审核、批准职责是否清晰，签批栏与受控文件要求是否一致，审批流程与角色是否前后矛盾或缺失。
- **时间线逻辑**：文档中出现的日期、版本号、生效日期、修订履历、各章节时间表述是否前后一致、顺序是否合理，是否存在「未生效已引用」「修订日期倒序」等明显矛盾。
...
9. **编审批与时间线（优先）**：对文档中出现的编制/审核/批准角色、签批与时间、版本与修订日期等须重点排查；发现职责不清、签批缺失、日期或版本前后矛盾时，优先按一致性或格式规范输出，severity 可适当提高"""
...
7. **编审批与时间线（优先）**：编制/审核/批准与时间、版本、修订日期等须重点排查；矛盾处优先输出，severity 可适当提高。
"""
...
class DocumentReviewer:
    ...
    def _review_long_document(
        ...
        # 长文档分块并发审核（限制并发数，避免压垮本地 Ollama 或 API 限流）
        max_workers = 3
