...
6. 若上下文提供【历史案例文档章节参考】，须进行「文档内容完整性」审核：须**逐条对照**参考章节清单，在待审文档中逐项确认是否存在；每缺失一条应有章节即单列一条完整性审核点，location 写明缺失的章节名称或应出现位置，不得合并笼统描述。**注意**：过往案例仅为通用结构/写法参考；案例中的软件/文件/发布等**版本号不得**作为【项目基本信息】或版本一致性核对依据，不得因待审文档版本与案例不一致而开出一致性审核点
...
6. 若审核点参考知识中包含【历史案例文档章节参考】，须**逐条对照**参考章节与待审文档；每缺失一条应有章节即单列一条完整性审核点，location 指明缺失章节或位置。过往案例仅供通用结构参考；**不得**以案例中的软件/文件/发布版本核对【项目基本信息】或待审文档版本一致性。
...
- **引用工具/组件版本与时间逻辑（优先）**：各文档中引用的**开发/测试/运行环境、现成软件/SOUP、第三方库、操作系统**等**具体版本号**，与其声称的**使用/测试/验证/编制/生效**日期是否一致；若某版本在文档时间点**尚未公开发布**（跨章节或跨文档同一工具版本与日期矛盾），须作为一致性/准确性审核点列出。版本发布日期须基于可核对来源，禁止臆造。
    ...
    def review_text(
        ...
        *,
        ...
        if review_context and review_context.get("basic_info_text"):
            ...
            if review_context.get("case_context_text"):
                context += (
                    "\n**版本说明**：若基本信息或待审文档含软件/文件/发布等版本号，"
                    "以本段【项目基本信息】与待审文档（及项目专属资料）为准；"
                    "**不得**用【过往项目案例参考】中的历史版本覆盖或核对版本一致性。"
                )
    ...
    def review_multi_document_consistency(
        ...
        try:
            if settings.is_cursor:
                ...
                _cl = resolve_client_llm(review_context=review_context)
                try:
                    response_content = invoke_chat_direct(
                        prompt_text or "",
